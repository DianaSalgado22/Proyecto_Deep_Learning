...
{
    static int16_t input_q[28][28];
    preprocess_input(image, input_q);

    static int16_t conv1_out[8 * 28 * 28];
    conv2d((int16_t*)input_q, 28, 28, _Conv1_weight, _Conv1_bias, conv1_out, 8, 3, 1);

    static int16_t pool1_out[8 * 14 * 14];
    maxpool2d(conv1_out, 28, 28, 8, pool1_out);

    static int16_t conv2_out[16 * 14 * 14];
    conv2d(pool1_out, 14, 14, _Conv2_weight, _Conv2_bias, conv2_out, 16, 3, 8);

    static int16_t pool2_out[16 * 7 * 7];
    maxpool2d(conv2_out, 14, 14, 16, pool2_out);

    int16_t dense_input[16 * 7 * 7];
    memcpy(dense_input, pool2_out, sizeof(dense_input));

    static int16_t fc1_out[32];
    linear(dense_input, _Linear1_weight, _Linear1_bias, 16 * 7 * 7, 32, fc1_out);

    static int16_t logits[10];
    linear(fc1_out, _Linear2_weight, _Linear2_bias, 32, 10, logits);
